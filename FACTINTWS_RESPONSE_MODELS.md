...
Taxy's `FactIntInvoiceResponse` parser keeps this wire contract separate from a minimal `AtInvoiceDomain` projection. Money is converted directly from decimal text to integer cents; date-only values remain `YYYY-MM-DD`. No FactInt DTO is forced into the fatshare wire model.

## Offline parser coverage

Synthetic fixtures cover a successful multi-invoice response, empty response, optional absent field, unknown element, malformed date, and SOAP fault. Fixtures use obviously synthetic identifiers and contain no real tax, document, credential or certificate data.

...
One-shot runtime evidence now refines that conclusion: `checkEntrega` accepted `2025` with status `0` and reported a delivered declaration. The official branch therefore proceeds to receipt handling and bypasses `infoUtilizador`. The previous direct `infoUtilizador` status `130` is most consistently explained as invalid eligibility for that delivery-flow branch, not rejection of the numeric year itself. This remains `PARTIAL` rather than exact causation because no server-side status documentation was provided.

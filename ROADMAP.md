# Taxy roadmap

## ACTIVE

- shell multi-módulo;
- IRS 2025 Continente e IRS 2026 Continente/Madeira/Açores;
- Categoria A standard;
- individual, monoparental, casado e união de facto;
- conjunta vs. separada;
- IRS Jovem end-to-end: histórico, elegibilidade, liquidação e comparação;
- referências IRS Jovem manualmente auditadas e loader oficial com rendimento
  isento;
- schema AT v3, audit trace tipado, intake/triage developer-only, comparação
  exata, privacidade e relatório absoluto;
- persistência local, rascunho retomável, cenários, breakdown e Validation Lab.

## NEXT

- adicionar liquidações oficiais reais anonimizadas da AT (zero presentes);
- validar humanamente casos transitórios anteriores a 2025 e arredondamentos
  diretamente contra demonstrações de liquidação;
- despesas de dependentes separadas na UX principal;
- guarda partilhada;
- Categoria B e pensões como módulos explícitos.

## FUTURE MODULES

- salário líquido;
- recibos verdes e Segurança Social;
- IVA e PPR avançado;
- mais-valias imobiliárias, mobiliárias e cripto;
- IMT, IMI e IUC;
- desemprego, parentalidade, prestações e reforma.

Itens “Em breve” na home não possuem motores falsos.
